...
#include <node/planner/ego_spatiotemporal_lattice_planning_node.h>
...
bool EgoSpatiotemporalLatticePlanningNode::initialize() {
...
boost::shared_ptr<planner::Snapshot> EgoSpatiotemporalLatticePlanningNode::createSnapshot(
...
void EgoSpatiotemporalLatticePlanningNode::executeCallback(
  ...
  const std::list<std::pair<ContinuousPath, double>> ego_traj =
    traj_planner_->planTraj(ego_policy.first, *snapshot);
...
int main(int argc, char** argv) {
  ...
  node::EgoSpatiotemporalLatticePlanningNodePtr planner =
    boost::make_shared<node::EgoSpatiotemporalLatticePlanningNode>(nh);
